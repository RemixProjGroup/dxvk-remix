...
## Workstream — SHARC: recover the update budget that exits to the sky (fork — 2026-09-16)

Two opt-in `rtx.sharc` options, both default off, both clearing the cache when changed, so
that an update path whose first bounce exits to the sky no longer inserts nothing:
`updatePrimaryVertex` deposits the path's primary vertex (RTXDI direct light plus the sampled
continuation), `updateSkyRetries` re-samples a first-bounce sky miss from a cosine lobe. Every
shader line is under `SHARC_UPDATE`; query and legacy stages are byte-identical to the previous
build. Record: `docs/SHARC-sky-budget-2026-09-16.md`.

- **`src/dxvk/shaders/rtx/algorithm/integrator_indirect.slangh`** — hook calls.
  *`sharcOnPathStart` after the launch state is final and `sharcOnSegmentEnd` after each traced
  segment, both under `#if SHARC_UPDATE`; the retry budget is a loop-local.*
- **`src/dxvk/shaders/rtx/pass/integrate/integrate_indirect_binding_indices.h`** — inline tweak.
  *Indices 48/49 for the shared material words, update stages only.*
- **`src/dxvk/shaders/rtx/pass/integrate/integrate_indirect_bindings.slangh`** — inline tweak.
  *Declares `SharedMaterialData0/1` under `SHARC_UPDATE` so the primary's eligibility can be
  judged from the same G-buffer words the direct pass decodes.*
- **`src/dxvk/rtx_render/rtx_pathtracer_integrate_indirect.cpp`** — fork-owned change.
  *Update-stage slots and binds for the two words; the four-slot deferred variant is chosen only
  when the primary's slot still fits.*
- **`src/dxvk/rtx_render/rtx_sharc.h / .cpp`**, **`src/dxvk/shaders/rtx/pass/sharc/*`** —
  fork-owned. *Options, flag word, UI, hook bodies, `sharcSeedVertex`.*
- **`RtxOptions.md`** — regenerated by the headless factory probe (also refreshes stale
  `rtx.sharc` rows).
...
- **DLSS-NR stays outside the stack.** `RtxContext::dispatchDlssNR` still runs in
  `rtx_context.cpp` immediately after upscaling and dust particles, ahead of every
  stack member. It is an upscaling-adjacent denoiser rather than a reorderable post
  effect, it runs its own auto-exposure dispatch, and it must not be gated by the
  stack's "Post FX Enabled" switch. Its return value is threaded into the stack as
  `updateAutoExposure` (a required parameter with no default) and forwarded to
  `RtxContext::dispatchToneMapping`, so auto exposure is still suppressed for the
  frame DLSS-NR already exposed.
- **`m_postFxIntermediateTexture` is an `AliasedResource` on this branch** (it shares
  memory with the DLSS-NR input), so every new binding claims it explicitly with
  `view(AccessType::Write)` or `view(AccessType::Read)` rather than taking a plain
  `Resource::view` member. The NTSC/VHS ping-pong alternates Write/Read per pass.
