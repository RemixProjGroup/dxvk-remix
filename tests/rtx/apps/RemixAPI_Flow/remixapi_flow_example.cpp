...
remixapi_FlowEmitterHandle g_flow_emitter = nullptr;
...
bool init(HWND hwnd) {
  ...
  // Enable Flow via config
  g_remix->SetConfigVariable("rtx.flow.enable", "1");

  // Create a light so the scene isn't pitch black
  {
  ...
  // Create a PhysX Flow emitter — fire-like sphere emitter
  {
    auto sphereExt = remixapi_FlowEmitterInfoSphereEXT {
      .sType = REMIXAPI_STRUCT_TYPE_FLOW_EMITTER_INFO_SPHERE_EXT,
      .pNext = nullptr,
      .radius = 3.0f,
    };
    auto emitterInfo = remixapi_FlowEmitterInfo {
      .sType = REMIXAPI_STRUCT_TYPE_FLOW_EMITTER_INFO,
      .pNext = &sphereExt,
      .hash = 0x100,
      ...
      .fuel = 2.0f,
      .smoke = 0.5f,
      .velocity = { 0.0f, 80.0f, 0.0f },
      .coupleRateTemperature = 10.0f,
      .coupleRateFuel = 10.0f,
      .coupleRateVelocity = 2.0f,
    };

    auto emitterHandle = g_remix->CreateFlowEmitter(emitterInfo);
    if (!emitterHandle) {
      throw std::runtime_error { "remix::CreateFlowEmitter() failed " + std::to_string(emitterHandle.status()) };
    }
    g_flow_emitter = emitterHandle.value();
  }
...
void render(uint32_t windowWidth, uint32_t windowHeight) {
  ...
  // Draw flow emitter — must be called every frame to keep it active
  g_remix->DrawFlowEmitterInstance(g_flow_emitter);
...
void destroy() {
  if (g_remix) {
    if (g_flow_emitter) {
      g_remix->DestroyFlowEmitter(g_flow_emitter);
    }
...
int main(int argc, char* argv[]) {
  ...
  auto wc = WNDCLASSEX {
    ...
    .hbrBackground = NULL,
    .lpszMenuName = NULL,
    .lpszClassName = "Remix API Flow Example",
    .hIconSm = NULL,
  };
  RegisterClassEx(&wc);
  ...
  HWND hwnd = CreateWindow(wc.lpszClassName, "Remix API Flow Example",
                            dwStyle,
